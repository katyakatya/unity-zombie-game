...
namespace TestTask_game
{
    public class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            while (!isGameOver)
            {
                Parallel.ForEach(players, new ParallelOptions() { MaxDegreeOfParallelism = 2 },
                    (player, i, j) =>
                    {
                        Thread thread = new Thread(page => TryToGuess(player, weightOfBasket, out isGameOver, ref lockObject));
                        thread.Start();
                    });
            }
