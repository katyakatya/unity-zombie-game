    ...
    public class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Parallel.ForEach(players, new ParallelOptions(),
                      (player) =>
                      {
                          Task task = new Task(() => TryToGuess(player, weightOfBasket));
                          task.Start();
                      });
            Console.ReadLine();
        }

        private static void TryToGuess(IPlayer player, int weightOfBasket)
        {
            ...
            while (!isWinner)
            {
                lock (objectToLock)
                {
                    ...
                    else
                    {
                        ...
                        Thread.CurrentThread.Join(Math.Abs(delta));
                    }
